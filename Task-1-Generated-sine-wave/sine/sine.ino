...
void setup(){
  Serial.begin(9600);
  while (!Serial);/*program start when open serial monitor*/
  ...
  /*แปลงโมเดลเป็น structure ของบอร์ด*/
  ...
  /*ดึงทุก operation ทุกตัวในไลบรารี่มาเซ็ค*/
...
void loop(){
  ...
  model_input->data.f[0] = x_val; /* ตอนนี้ model = sine( )*/
  ...
  if (i >= kInferencesPerCycle){
    i = 0; /*กันค่า i overflow เมื่อ i >= 1024 ให้รีเซ็ทค่า i*/
  }
}
